...
## 0.2.2.post2（工具链与检查器加固）

- **uninstall 框架包防护**（真 bug 修复）: 某平台插件只剩一个应用挂载时, `aic uninstall`
  曾把 `aic/extensions/platform/*` 框架包误判为"专属插件"连带删除（卸载最后一个应用即删框架）——
  已修: 专属判定排除 `aic.` 模块 + 删除层兜底拒绝 `aic/` 路径。
- **旁路检查默认无豁免**: `_DEFAULT_EXCLUDED` 清空（整仓按契约扫描）;
  业务遗留债需豁免时经 `KIT_EXCLUDED_DIRS` 或 `check_bypass_imports(exclude=...)` 精确到文件。
- **仓库示例精简**: 移除示例业务应用/插件（框架发布包本就只含 aic, 不含业务——
  仓库代码层面的示例清理, 不影响发布包内容）; 回归套件改为自造夹具（不绑定具体业务应用）。
