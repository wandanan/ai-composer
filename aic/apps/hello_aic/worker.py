"""apps/hello_aic/worker.py — Celery worker 入口（空档位骨架, aic.tools.init 生成）。
...
celery_app = Celery("hello_aic", broker=_BROKER, backend=_BROKER)
...
def main() -> None:
    ...
    subprocess.run([
        sys.executable, "-m", "celery", "-A", "apps.hello_aic.worker", "worker",
        "--loglevel=info", "--pool=threads", f"--concurrency={concurrency}",
        "-n", "hello_aic@%h", "-Q", "default",
    ], check=False)
