...
规则: 想把什么带进模板 → 先 promote（python -m aic.tools.promote XxxPlugin --yes）
     template 只执行你的上浮声明; 未上浮的插件（即使多应用挂载）不带走,
     只列出提示。复制粒度为插件包（包完整性优先, 包内未上浮插件不挂载）。

用法:
    python -m aic.tools.template review                 # 预演: 清单
    python -m aic.tools.template review --out ~/my-tpl  # 提取到指定目录
"""
...
_TEMPLATE_CHECK = '''"""test/template_check.py — 模板自检（aic.tools.template 生成）。
...
def _write_hello_aic(out_dir: str, graph: dict) -> None:
    ...
    from aic.tools.init import (APP_CONFIG, APP_MAIN, APP_SHELL, APP_TASKS,
                            APP_WORKER)
    name = "hello_aic"
