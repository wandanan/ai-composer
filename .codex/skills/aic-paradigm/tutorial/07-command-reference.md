...
| 查框架有什么可用能力 | `aic caps` | 平台服务表（key/换法/特性）→ ctx.get 消费；业务插件 → 挂载 |
...
输出四段：**平台服务**（key + 插件 + 换法 + 特性，`ctx.get(key)` 直接消费）、**业务插件**（provides + 特性，profile.py 挂载）、**声明工具**（UTILITY_MODULES 白名单，纯函数直接 import）、**引擎**（agentLoop 实现选择）。

**用途**：写任何能力前的第一步——四问判断源（语义/形状/实现/行为）全部在输出里，不用读插件源码。配合能力阶梯使用（平台服务 → 业务插件 → 声明工具 → 扩展优先 → 才写新插件）。
