...
def _scan_ast(tree: ast.AST, rel: str) -> list[str]:
    ...
    for node in ast.walk(tree):
        # a. import extensions.* → 直接拿实现（profile.py 等装配组已豁免）
        if isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name == "extensions" or alias.name.startswith("extensions."):
                    problems.append(f"{rel} import 插件实现: {alias.name}")
        elif isinstance(node, ast.ImportFrom):
            if node.module and (node.module == "extensions"
                                or node.module.startswith("extensions.")):
