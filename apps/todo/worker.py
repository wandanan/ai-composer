"""apps/todo/worker.py — Celery worker 入口（空档位骨架, aic.tools.init 生成）。
...
celery_app = Celery("todo", broker=_BROKER, backend=_BROKER)
...
def main() -> None:
    ...
    subprocess.run([
        sys.executable, "-m", "celery", "-A", "apps.todo.worker", "worker",
