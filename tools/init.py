...
APP_MAIN = '''"""{name} — {name} 应用壳（tools.init 生成）。
...
APP_SHELL = '''"""apps/{name}/shell.py — 装配（tools.init 生成）。
...
APP_PROFILE = '''"""apps/{name}/profile.py — 插件组合（tools.init 生成）。
...
APP_WORKER = '''"""apps/{name}/worker.py — Celery worker 入口（空档位骨架, tools.init 生成）。
...
BUSINESS_INIT = '''"""extensions/business/{name} — {name} 业务插件（tools.init 生成）。"""
