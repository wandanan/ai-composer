...
影响分析直接计算（AST 扫描声明式元数据, 复用 tools.graph.build_graph 函数,
不依赖任何命令的前置产物）——卸载是"算清楚再删", 不是 rm -rf。

用法:
    python -m tools.uninstall my-app                     # 预演: 影响清单
    python -m tools.uninstall my-app --yes               # 实际卸载（壳 + 专属插件）
    python -m tools.uninstall --plugin DemoPlugin --yes  # 卸载插件包（无消费方闭包）
...
def _root() -> str:
    """项目根 = cwd（与 tools.graph 一致; KIT_PROJECT_ROOT 测试覆盖）。"""
