...
先看清清单, 确认无误后加 --yes 真正执行）:
    python -m tools.promote MyPlugin                 # 预演（默认）: 只看清单, 什么都不改
    python -m tools.promote MyPlugin --yes           # 真正上浮（默认到 platform 公共惯例位）
    python -m tools.promote MyPlugin --to business   # 上浮到领域惯例位（共享领域插件）
    python -m tools.promote MyPlugin --to my_plugins # 上浮到隐式插件区任意目录
    例: python -m tools.promote ReviewPlugin
...
def _write_public(path: str, cls: str) -> bool:
    ...
    if not m:
        return False
    insert = m.group(0) + "\n    PUBLIC = True  # 公共插件: 不随任何应用卸载删除（tools.promote 写入）"
    with open(path, "w", encoding="utf-8") as f:
